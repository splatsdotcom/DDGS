...
TORCH_LIBRARY(ddgs, m) 
{
	...
	m.def(
		"forward(__torch__.torch.classes.ddgs.Settings settings, Tensor means, Tensor scales, Tensor rotations, Tensor opacities, Tensor harmonics) -> (Tensor, int, Tensor, Tensor, Tensor)",
		&ddgs_forward
	);
	m.def(
